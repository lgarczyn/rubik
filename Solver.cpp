...
State Solver::get_smallest_state() {
	...
	//while iterator list is empty and iterator has not reached end, remove list
...
Solver::Result Solver::step() {
	...
	if (_openCount <= 0)
		throw std::logic_error("A No opened state, scount is " + std::to_string(_openCount));
	...
	//	if there are similar ids with insuficient scores, they will get ignored but
	//	if the nodes are found again, they will be explored if no progress has been made
	...
	if (distance > e.get_distance())
		throw std::logic_error("stored distance is greater than node distance: " + std::to_string(distance) + ">" + std::to_string(e.get_distance()));
	...
	for (State s:candidates) {
		...
		if (State::get_index(s) > 20 * score_multiplier)//TODO maybe 19 instead of 20? //TODO 11 instead of 20 for corners?
			continue;
		...
		//std::cout << "storing: " << s.get_id().corners << ":" << childmoves.size() << std::endl;
