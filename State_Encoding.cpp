...
namespace Encoding {
	...
	static constexpr inline uint get_fact_value(uchar cube_id, uchar *values, int len) {
		uchar r = values[cube_id];
		move_values_down(values, cube_id, len);
		return r;
	}

	static constexpr inline void set_corner_rot(uchar& up, uchar& fb, uchar& rl, uchar i) {
		if (i == 0) {
			up = 0;
			fb = 1;
			rl = 2;
	...
	static constexpr void set_cube_corners_rot(Cube& cube, const DataCorners& data) {
		set_corner_rot(cube[Index_U][0][0].rot_id, cube[Index_L][0][0].rot_id, cube[Index_B][0][2].rot_id, data[0].rot_id);
		set_corner_rot(cube[Index_U][0][2].rot_id, cube[Index_B][0][0].rot_id, cube[Index_R][0][2].rot_id, data[1].rot_id);
		set_corner_rot(cube[Index_U][2][0].rot_id, cube[Index_F][0][0].rot_id, cube[Index_L][0][2].rot_id, data[2].rot_id);
		set_corner_rot(cube[Index_U][2][2].rot_id, cube[Index_R][0][0].rot_id, cube[Index_F][0][2].rot_id, data[3].rot_id);
		set_corner_rot(cube[Index_D][0][0].rot_id, cube[Index_L][2][2].rot_id, cube[Index_F][2][0].rot_id, data[4].rot_id);
		set_corner_rot(cube[Index_D][0][2].rot_id, cube[Index_F][2][2].rot_id, cube[Index_R][2][0].rot_id, data[5].rot_id);
		set_corner_rot(cube[Index_D][2][0].rot_id, cube[Index_B][2][2].rot_id, cube[Index_L][2][0].rot_id, data[6].rot_id);
	...
	static constexpr void set_cube_corners_pos(Cube& cube, const DataCorners& data) {
		...
		cube[Index_U][2][2].cube_id = cube[Index_R][0][0].cube_id = cube[Index_F][0][2].cube_id = data[3].cube_id;
		cube[Index_D][0][0].cube_id = cube[Index_L][2][2].cube_id = cube[Index_F][2][0].cube_id = data[4].cube_id;
		cube[Index_D][0][2].cube_id = cube[Index_R][2][0].cube_id = cube[Index_F][2][2].cube_id = data[5].cube_id;
		cube[Index_D][2][0].cube_id = cube[Index_L][2][0].cube_id = cube[Index_B][2][2].cube_id = data[6].cube_id;
		cube[Index_D][2][2].cube_id = cube[Index_R][2][2].cube_id = cube[Index_B][2][0].cube_id = data[7].cube_id;
	}
	...
	static constexpr void set_cube_borders_rot(Cube& cube, const DataBorders& data) {
		...
		cube[Index_D][2][1].rot_id = data[11].rot_id;	cube[Index_B][2][1].rot_id = !data[11].rot_id;
	}

	static constexpr void set_cube_borders_pos(Cube& cube, const DataBorders& data) {
		cube[Index_U][0][1].cube_id = cube[Index_B][0][1].cube_id = data[0].cube_id;
	...
	static constexpr inline void swap_data(DataBorders& data, int a, int b, int c, int d) {
	...
	static constexpr inline void swap_data(DataCorners& data, int a, int b, int c, int d) {
	...
	template<size_t T>
	static constexpr inline void swap_data_poles(std::array<Square, T>& data, int a, int b, int c, int d) {
		Square t = data[d];
		data[d] = data[c];
		data[c] = data[b];
		data[b] = data[a];
	...
	static constexpr inline void move_values_left(uchar* values, uchar pos, int len) {
	...
	static constexpr inline uchar get_value_fact(uint s, uchar* values, uchar pos, uchar len) {
		if (pos == 1)
			return (values[0]);
	...
	static constexpr void set_data_corners_rot(DataCorners& data, uint id) {
		data[7].rot_id = id % 3; id /= 3;
		data[6].rot_id = id % 3; id /= 3;
		data[5].rot_id = id % 3; id /= 3;
		data[4].rot_id = id % 3; id /= 3;
		data[3].rot_id = id % 3; id /= 3;
		data[2].rot_id = id % 3; id /= 3;
		data[1].rot_id = id % 3; id /= 3;
		data[0].rot_id = id; //s % 3)); s /= 3;
	}

	static constexpr void set_data_borders_rot(DataBorders& data, uint id) {//TODO optimize
		uint s = id;

		data[11].rot_id= s % 2; s /= 2;
		data[10].rot_id= s % 2; s /= 2;
		data[9].rot_id = s % 2; s /= 2;
		data[8].rot_id = s % 2; s /= 2;

		data[7].rot_id = s % 2; s /= 2;
		data[6].rot_id = s % 2; s /= 2;
		data[5].rot_id = s % 2; s /= 2;
		data[4].rot_id = s % 2; s /= 2;

		data[3].rot_id = s % 2; s /= 2;
		data[2].rot_id = s % 2; s /= 2;
		data[1].rot_id = s % 2; s /= 2;
...
constexpr Finder State::_calculate_finder(const Cube &cube) { return Encoding::_calculate_finder(cube);}
constexpr Cube State::cube_from_data(const Data& data) { return Encoding::cube_from_data(data);}
constexpr ID State::id_from_data(const Data& data){ return Encoding::id_from_data(data);}
