...
/*   Updated: 2018/04/09 22:35:55 by lgarczyn         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
constexpr int Heuristics::SquareDistance(Coord a, Coord b) {
    ...
    if (isborder_a != isborder_b) return 9;
    bool iscenter_a = (a.c == 1 && a.l == 1);
    bool iscenter_b = (b.c == 1 && b.l == 1);
    if (iscenter_a || iscenter_b) return 9;
    if (a.f == b.f) {
	if (a.c == b.c && a.l == b.l) return 0;
	return 1;
    }

    // If a and b are on opposite sides
    if (a.f == _get_opposite(b.f)) {
	if (a.c == b.c && a.l == b.l) return 1;
	if (a.c == 2 - b.c && a.l == 2 - b.l) return 1;
	return 2;
    }
    ...
    switch (a.f) {
	case Index_U:
	    if (b.f == Index_L) clockwise = false;
	    break;
	case Index_R:
	    if (b.f == Index_U) clockwise = false;
	    break;
	case Index_D:
	    if (b.f == Index_R) clockwise = false;
	    break;
	case Index_L:
	    if (b.f == Index_D) clockwise = false;
	    break;
    }
